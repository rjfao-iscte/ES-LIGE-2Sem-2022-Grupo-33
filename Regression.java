...
public abstract class Regression {
    ...
    public static double[] getPolynomialRegression(XYDataset dataset, 
        ...
        double sumX = sumX(dataset, series, itemCount);
		int validItems = 0;
		...
		sumY = sumY(validItems, data, sumY);
        ...
        yObsSquare = yObsSquare(validItems, data, meanY, yObsSquare);
		double rSquare = yRegSquare / yObsSquare;
        result[equations] = rSquare;
        return result;
    }

	private static double yObsSquare(int validItems, double[][] data, double meanY, double yObsSquare) {
		for (int item = 0; item < validItems; item++) {
	...
	private static double sumX(XYDataset dataset, int series, int itemCount) {
		int validItems = 0;
		double[][] data = new double[2][itemCount];
		for (int item = 0; item < itemCount; item++) {
			double x = dataset.getXValue(series, item);
	...
	private static double sumY(int validItems, double[][] data, double sumY) {
		for (int item = 0; item < validItems; item++) {
			sumY += data[1][item];
		}
		return sumY;
	...
	private static double yRegSquare(int validItems, double[][] data, int equations, int coefficients, double[] result,
		...
		result = result(equations, coefficients, result, matrix);
		double meanY = sumY / validItems;
		double yRegSquare = 0.0;
		for (int item = 0; item < validItems; item++) {
			double yCalc = yCalc(data, equations, result, item);
			yRegSquare += Math.pow(yCalc - meanY, 2);
		}
		return yRegSquare;
	}

	private static double[] result(int equations, int coefficients, double[] result, double[][] matrix) {
		for (int eq = equations - 1; eq > -1; eq--) {
			double value = value(coefficients, result, matrix, eq);
			result[eq] = value / matrix[eq][eq];
		}
		return result;
	}

	private static double yCalc(double[][] data, int equations, double[] result, int item) {
		double yCalc = 0;
		for (int eq = 0; eq < equations; eq++) {
	...
	private static double value(int coefficients, double[] result, double[][] matrix, int eq) {
